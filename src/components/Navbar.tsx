...
export default function Navbar() {
  ...
  const isHome = location.pathname === '/' || location.pathname === '' || location.pathname === '/apartments';

  const handleApartmentsClick = (e: React.MouseEvent) => {
    e.preventDefault();
    if (!isHome) {
      navigate('/');
      setTimeout(() => {
  ...
  const bgClass = scrolled || !isHome ? 'bg-[#FAF9F6] shadow-sm text-[#2D2D2D] border-b border-[#E5E1D8]' : 'bg-transparent text-white border-b border-transparent';
  const navTextColor = scrolled || !isHome ? 'text-[#2D2D2D]' : 'text-white';
    ...
    <nav className={`fixed w-full z-50 transition-all duration-300 ${bgClass}`}>
      <div className="max-w-7xl mx-auto px-6 md:px-12 py-6 h-auto flex items-center justify-between">
        <div className="flex flex-col">
          <Link to="/" onClick={handleHomeClick} className="text-2xl font-light tracking-[0.2em] uppercase font-serif leading-none">Le Due Rose</Link>
          <span className={`text-[10px] uppercase tracking-[0.3em] ml-0.5 mt-1 transition-colors duration-300 ${scrolled || !isHome ? 'text-[#8C8578]' : 'text-white/70'}`}>Ischia • Italy</span>
        </div>
        
        {/* Desktop Menu */}
        <div className="hidden md:flex items-center space-x-10">
          <Link to="/" onClick={handleHomeClick} className={`text-xs uppercase tracking-widest font-medium hover:text-[#B59461] transition-colors duration-300 ${navTextColor}`}>{t.home}</Link>
          <button onClick={handleApartmentsClick} className={`text-xs uppercase tracking-widest font-medium hover:text-[#B59461] transition-colors duration-300 ${navTextColor}`}>{t.apartments}</button>
          <Link to="/faq" className={`text-xs uppercase tracking-widest font-medium hover:text-[#B59461] transition-colors duration-300 ${navTextColor}`}>{t.faq}</Link>
          <Link to="/contact" className={`text-xs uppercase tracking-widest font-medium hover:text-[#B59461] transition-colors duration-300 ${navTextColor}`}>{t.contact}</Link>
        ...
        {mobileMenuOpen && (
          ...
          >
            <div className="px-6 py-6 flex flex-col gap-6">
              <Link to="/" onClick={handleHomeClick} className="text-xs uppercase tracking-widest font-medium">{t.home}</Link>
              <button onClick={handleApartmentsClick} className="text-xs uppercase tracking-widest font-medium text-left">{t.apartments}</button>
